...
如果你需要托管式的紫微斗数对话解读能力，`iztro` 也提供可选的托管 Chat API。该服务针对对话式命盘解读做了优化，并会在内部调用 `iztro` 获取紫微斗数排盘数据。

托管 API 与开源 `iztro` 包相互独立，使用时需要 API key。你可以在 [platform.iztro.com](https://platform.iztro.com) 申请和管理 API key，并在 [api-doc.iztro.com](https://api-doc.iztro.com) 查看 API 文档。

推荐的集成方式是 Chat Session API：先创建托管会话，再向该会话发送用户消息。这样 API 可以为你的用户保留上下文。
...
JavaScript 和 Python 示例见 [`examples/chat-api`](./examples/chat-api)。如果你已有兼容 OpenAI Chat Completions 的客户端，示例中也包含 `/v1/chat/completions` 的用法。
