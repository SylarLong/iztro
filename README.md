...
| **`iztro-ziwei-v3-fast`** | 聚焦、低延迟的单人紫微分析；对外响应与 `iztro-ziwei-v3` 一致 | 出生日期、出生时间、性别和分析主题 |
| **`iztro-qimen-v3-fast`** | 聚焦、低延迟的奇门决策与应期分析；对外响应与 `iztro-qimen-v3` 一致 | 事情背景、一个明确问题和问事时刻；**不需要出生信息** |
...
- `iztro-ziwei-v3-fast` 是 `iztro-ziwei-v3` 的聚焦低延迟版本。
- `iztro-qimen-v3-fast` 是 `iztro-qimen-v3` 的聚焦低延迟版本。

两者都沿用对应 v3 模型的响应结构和用户可见 Iztro 计算活动信息，不会引入另一套对外名称。
