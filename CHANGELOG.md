...
- ✨ 改进（enhancement）
  ...
  - 为韩文和越南语中的同名星曜新增带汉字的限定别名，用于准确反查星曜。
  - 将武曲和岁破的英文名分别调整为 `warrior` 和 `breaker`，避免与将军和大耗重名。

- 🛠️ 修复（fix）

  - 修正太阳、太阴和七杀在酉宫的亮度数据。
  - 修复以生日为分界点时，后续年份在农历生日同月、生日之后虚岁少算一岁的问题。
  - 修复 `horoscope()` 显式传入早子时索引 `0` 时，被误判为未传入并使用目标时间中的小时数的问题。
  - 修复运限查询未遵守晚子时日期分界配置的问题。
  - 修复运限宫位查询和分析方法不支持混合语言宫名输入的问题。
