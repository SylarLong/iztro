...
async function api(path, body) {
  const response = await fetch(`https://api.iztro.com${path}`, {
    method: 'POST',
    headers: {
...
const session = await api('/api/platform/sessions', {
  external_user_id: 'user_123',
  system_prompt_override: '用简洁中文回答，避免过度术语，并在最后给出可继续追问的方向。',
});

const reply = await api(`/api/platform/sessions/${session.session_id}/messages`, {
