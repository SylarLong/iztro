...
| **`iztro-ziwei-v3-fast`** | 聚焦、低延遲的單人紫微分析；對外回應與 `iztro-ziwei-v3` 一致 | 出生日期、出生時間、性別和分析主題 |
| **`iztro-qimen-v3-fast`** | 聚焦、低延遲的奇門決策與應期分析；對外回應與 `iztro-qimen-v3` 一致 | 事情背景、一個明確問題和問事時刻；**不需要出生資訊** |
...
- `iztro-ziwei-v3-fast` 是 `iztro-ziwei-v3` 的聚焦低延遲版本。
- `iztro-qimen-v3-fast` 是 `iztro-qimen-v3` 的聚焦低延遲版本。

兩者都沿用對應 v3 模型的回應結構和使用者可見 Iztro 計算活動資訊，不會引入另一套對外名稱。
