...
| **`iztro-ziwei-v3-fast`** | 回應更快、消耗更少 Token、調用成本更低的單人紫微分析；對外回應與 `iztro-ziwei-v3` 一致 | 出生日期、出生時間、性別和分析主題 |
| **`iztro-qimen-v3-fast`** | 回應更快、消耗更少 Token、調用成本更低的奇門決策與應期分析；對外回應與 `iztro-qimen-v3` 一致 | 事情背景、一個明確問題和問事時刻；**不需要出生資訊** |
...
- 根據問事時刻分析**一件具體事情**，給出結論、主要依據、阻力與行動建議。
- 當使用者問「什麼時候」或時間是結論關鍵時，給出真實日曆中的候選觸發窗口。
...
- `iztro-ziwei-v3-fast` 回應更快、消耗更少 Token、調用成本更低。
- `iztro-qimen-v3-fast` 回應更快、消耗更少 Token、調用成本更低。

兩者的回應結構和使用者可見資訊都與對應的 v3 模型一致。
...
如果你需要紫微或奇門對話能力，可以使用 iztro Chat API。使用時需要 API key，你可以在[開發者文檔](https://api-doc.iztro.com)查看完整介面，並透過[模型指南](https://api-doc.iztro.com/sdk/qimen)比較紫微與奇門。
...
推薦將帶版本號的 `iztro-v2.6.0.min.js` 用 `script` 標簽引入 HTML 文件使用。構建產物同時保留 `iztro.min.js`，用於兼容舊版本的引用方式。
